...
describe("maintenance password reset endpoints", () => {
  let app: Express;
  ...
  it("requests a password reset and returns the generic message envelope", async () => {
    ...
    const response = await request(app)
      ...
      .send({ email: "repair.admin@iphoneman.test" })
      .expect(200);

    expect(response.body.data.message).toBe("If an account exists for this email, a reset link has been sent.");
    expect(response.body.error).toBeNull();
    expect(requestPasswordResetMock).toHaveBeenCalledWith({ email: "repair.admin@iphoneman.test" });
  });
